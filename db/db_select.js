...
client.connect(async function(er) {
	...
	// const res_ssessions = await client.query(SELECT_SESSIONS);
	// const res_profiles = await client.query(SELECT_PROFILES);
	const res_profiles = await client.query(SELECT_PROFILES_WHERE, [1]);
	// const res_orders = await client.query(SELECT_ORDERS);

	// console.log('sessions', JSON.stringify(res_ssessions));
	console.log('profiles', JSON.stringify(res_profiles.rows));
